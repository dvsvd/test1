	...
	std::thread([=, Sender = (Sender)] () mutable {
		try
		{
			...
			case COMMAND_PRIVATE_MESSAGE:
				...
				// ID
				// AddrLen
			case COMMAND_TRANSMIT_REQUEST:
				// NameLen
				// ID
				// FileSizeLow
				// FileSizeHigh
				// AddrLen
			case COMMAND_TRANSMIT_RESPONSE:
				// NameLen
				// ID
				// Transmit response
				// AddrLen
...
{
	std::vector<WSABUF> buf(6);
	DWORD iNum = 0;
	p.Data.push_back(Sender.ID);
	p.Data.push_back(wcslen(Sender.addr));
	p.DataLen = p.Data.size();
	buf[0].buf = (CHAR*)&p.Code;
	buf[0].len = 4;
	buf[1].buf = (CHAR*)&p.DataLen;
	buf[1].len = 4;
	buf[2].buf = (CHAR*)p.Data.data();
	buf[2].len = p.DataLen * 4;
	buf[3].buf = (CHAR*)Sender.addr;
	buf[3].len = p.Data[2] * sizeof(wchar_t);
	buf[4].buf = (CHAR*)p.Name.c_str();
	buf[4].len = p.Data[0] * sizeof(wchar_t);
	buf[5].buf = (CHAR*)p.Message.c_str();
	buf[5].len = p.Message.size() * sizeof(wchar_t);
	p.Code = htonl(p.Code);
...
void TcpServer::SendPrivate(const ClientInfo& Sender, Packet& p)
{
	std::vector<WSABUF> buf(6);
	DWORD iNum = 0;
	int32_t l_Code = p.Code;
	buf[0].buf = (CHAR*)&p.Code;
	buf[0].len = 4;
	auto RIt = FindByID(p.Data[1]);
	if (RIt != ClientList.end())
	{
		...
		buf[2].len = p.DataLen * 4;
		buf[3].buf = (CHAR*)Sender.addr;
		buf[3].len = p.Data.back() * sizeof(wchar_t);
		buf[4].buf = (CHAR*)p.Message.c_str();
		buf[4].len = p.Message.size() * sizeof(wchar_t);
		...
		if (l_Code == COMMAND_PRIVATE_MESSAGE)
		{
			// Вернуть отправителю
			// Заменяем инфоррмацию об отправителе информацией о получателе
			// Не нужно возвращать имя
			p.Code = htonl(COMMAND_PM_RETURN);
			p.Data[1] = htonl(RIt->ID);
			p.Data[2] = htonl(wcslen(RIt->addr));
			buf[3].buf = (CHAR*)RIt->addr;
			buf[3].len = wcslen(RIt->addr) * sizeof(wchar_t);
			if (WSASend(Sender.s, buf.data(), buf.size() - 1, &iNum, 0, NULL, NULL) == SOCKET_ERROR)
			{
				throw std::runtime_error(string("send failed. Code: ") + std::to_string(WSAGetLastError()));
			}
		}
	...
	else
	{
		...
		buf[1].buf = (CHAR*)&p.DataLen;
		buf[1].len = 4;
		buf[2].buf = (CHAR*)L"No client with such ID found.";
		buf[2].len = 29 * sizeof(wchar_t);
		if (WSASend(Sender.s, buf.data(), 2, &iNum, 0, NULL, NULL) == SOCKET_ERROR)
		{
			throw std::runtime_error(string("send failed. Code: ") + std::to_string(WSAGetLastError()));
		}
	}
